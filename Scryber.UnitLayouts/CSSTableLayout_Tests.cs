  ...
  <div style=""display:table; width:600pt;"">
    <div style=""display:table-row;"">
      ...
      <div style=""display:table-cell; height:50pt; padding:4pt;"">CSS B</div>
        ...
        // -----------------------------------------------------------------------
        // Cell style propagation — padding set on source panel flows to cell
        // -----------------------------------------------------------------------

        [TestCategory(TestCategory)]
        [TestMethod()]
        public void CSSTable_CellPadding_Propagated()
        {
            // Two single-cell tables: one with no padding, one with 20pt padding.
            // The padded cell should produce a taller row block because padding adds
            // to the outside of the content height.

            // --- Table A: no padding ---
            var docA   = CreateDoc(out var pgA);
            var tableA = CreateCSSTable(pgA);
            var rowA   = AddRow(tableA);
            var cellA  = new Panel();
            cellA.Style.Position.DisplayMode = DisplayMode.TableCell;
            cellA.Style.Padding.All = 0;
            cellA.Contents.Add(new Label { Text = "A" });
            rowA.Contents.Add(cellA);

            PDFLayoutDocument layoutA = null;
            docA.LayoutComplete += (s, e) => layoutA = e.Context.GetLayout<PDFLayoutDocument>();
            using (var ms = DocStreams.GetOutputStream("CSSTable_CellPadding_A.pdf"))
                docA.SaveAsPDF(ms);

            // --- Table B: 20pt padding ---
            var docB   = CreateDoc(out var pgB);
            var tableB = CreateCSSTable(pgB);
            var rowB   = AddRow(tableB);
            var cellB  = new Panel();
            cellB.Style.Position.DisplayMode = DisplayMode.TableCell;
            cellB.Style.Padding.All = 20;
            cellB.Contents.Add(new Label { Text = "B" });
            rowB.Contents.Add(cellB);

            PDFLayoutDocument layoutB = null;
            docB.LayoutComplete += (s, e) => layoutB = e.Context.GetLayout<PDFLayoutDocument>();
            using (var ms = DocStreams.GetOutputStream("CSSTable_CellPadding_B.pdf"))
                docB.SaveAsPDF(ms);

            Assert.IsNotNull(layoutA);
            Assert.IsNotNull(layoutB);

            var tableBlockA = layoutA.AllPages[0].ContentBlock.Columns[0].Contents[0] as PDFLayoutBlock;
            var tableBlockB = layoutB.AllPages[0].ContentBlock.Columns[0].Contents[0] as PDFLayoutBlock;

            Assert.IsNotNull(tableBlockA, "Table A block should exist");
            Assert.IsNotNull(tableBlockB, "Table B block should exist");

            var rowBlockA = tableBlockA.Columns[0].Contents[0] as PDFLayoutBlock;
            var rowBlockB = tableBlockB.Columns[0].Contents[0] as PDFLayoutBlock;

            Assert.IsNotNull(rowBlockA, "Row A block should exist");
            Assert.IsNotNull(rowBlockB, "Row B block should exist");

            double heightA = rowBlockA.TotalBounds.Height.PointsValue;
            double heightB = rowBlockB.TotalBounds.Height.PointsValue;

            // Padding 20pt top + 20pt bottom = 40pt extra.
            Assert.IsTrue(heightB >= heightA + 30,
                $"Cell with padding=20 should be at least 30pt taller than cell with padding=0. Got: noPadding={heightA}, withPadding={heightB}");
        }
