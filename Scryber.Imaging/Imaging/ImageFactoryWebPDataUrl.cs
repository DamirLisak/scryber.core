...
{
    /// <summary>
    /// Handles WebP images from a data url e.g. data:image/webp;base64,.....
    /// </summary>
    public class ImageFactoryWebPDataUrl : ImageFactoryWebP
    {
        private static readonly Regex GifDataMatch = new Regex("^\\s*data:image/webp;", RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.Compiled);
        private const string GifDataName = "WebP inline data image factory";
        private const bool GifDataShouldCache = false;

        //prefix is "data:image/png;base64,"
        
        public ImageFactoryWebPDataUrl() : this(GifDataMatch, GifDataName, GifDataShouldCache)
        {}
        
        
        protected ImageFactoryWebPDataUrl(Regex match, string name, bool shouldCache)
        ...
        {
            var bin = Utilities.DataUrlHelper.ExtractBase64Data(path, "image/webp");
